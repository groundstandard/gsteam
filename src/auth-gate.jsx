    ...
    <div style={{
      ...
      <div style={{
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 6 }}>
          ...
          />
          {/* Role switcher: only visible to the Owner. Everyone else sees
              only their own view — Account Manager / Client Associate / etc. */}
        ...
        <div style={{ display: 'flex', alignItems: 'baseline', justifyContent: 'space-between' }}>
          ...
          {/* Persona switcher (UserPicker) — locked to Owner only.
              Non-owner users always see their own data, not someone else's. */}
...
function RoleSwitcher({ role, onChange, theme }) {
  // Owner-only switcher to preview the app as each persona category.
  // Labels match the agreed names: CA = Client Associate, Sales = AM/RDR, Admin = Admin/FI.
  const PILLS = [
    { value: 'CA',    short: 'CA',    label: 'Client Associate' },
    { value: 'Sales', short: 'AM',    label: 'Account Manager' },
    { value: 'Admin', short: 'Admin', label: 'Admin' },
  ];
  return (
    <div style={{ display: 'flex', background: theme.rule, borderRadius: 999, padding: 2, gap: 0 }}>
      {PILLS.map(p => (
        <button key={p.value} onClick={() => onChange(p.value)} title={p.label} style={{
          ...
          background: role === p.value ? theme.surface : 'transparent',
          color: role === p.value ? theme.ink : theme.inkMuted,
          cursor: 'pointer', fontFamily: 'inherit',
          boxShadow: role === p.value ? '0 1px 2px rgba(0,0,0,0.06)' : 'none',
        }}>{p.short}</button>
      ))}
    </div>
  );
